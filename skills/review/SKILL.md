...
Review the report at .claude/review-reports/{page-name}.md, then:
  ...
  - Re-run /review {page} to auto-fix remaining issues
```
...
{general_heuristics from skills/ux/SKILL.md}
...
```typescript
// Generated by /review on 2026-03-07
...
Configurable based on machine capability (detected by `/detect`):
...
`.claude/review-config.json` — populated incrementally on first run and via `/detect`:
...
| General heuristics | `skills/ux/SKILL.md` |
| Project heuristics | `.claude/ui-heuristics.md` |

---

**Status**: ACTIVE
**Related Skills**: `ux-heuristics` (general heuristics), `web-design-guidelines` (design rules), `detect` (UI convention detection + story generation)
