...
**Invocation modes**:
- **On-demand**: `/health` — full repo scan
...
Read `.pm/health.yml` if it exists. If not, use built-in defaults.

```bash
# Check for config
if [ -f .pm/health.yml ]; then
  echo "Using .pm/health.yml"
else
...
These apply when no `.pm/health.yml` exists:
...
```
1. Read each subagent prompt from skills/health/subagent-prompts/
...
## Config Reference (.pm/health.yml)

Full config file with all options:

```yaml
# .pm/health.yml — Code health audit configuration
...
**Status**: ACTIVE
**Related Skills**: `test` (post-audit hook), `plan` (pre-task baseline)
**Config**: `.pm/health.yml` (optional)
